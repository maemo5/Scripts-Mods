...
private["_recipeClassName", "_quantityToCraft", "_quantityCrafted", "_metSideConditions", "_recipeConfig", "_returnedItems", "_interactionModelGroupClassName", "_components", "_tools", "_equippedMagazines", "_addedItems", "_concreteMixer",
"_toolItemClassName", "_equippedToolQuantity", "_interactionModelGroupModels", "_foundObject", "_i", "_hasAllComponents", "_componentQuantity", "_componentItemClassName", "_equippedComponentQuantity", "_groundHolder", "_returnedItemQuantity",
"_returnedItemClassName", "_feedbackMessage", "_returnedItemName", "_chance", "_breakage", "_toBreak", "_tools", "_weld", "_gen", "_toolTrolley"];
...
_chance = 7; //1 in x chance of Duct tape or fiberglass being consumed if used.
_chance2 = 20;
{
...
{
	if !([player, 5] call ExileClient_util_world_isFireInRange) then 
	{
		_metSideConditions = false;
	};
};
///welder
if ( getNumber(_recipeConfig >> "requiresWeld") == 1 ) then
{
...
///generator
if ( getNumber(_recipeConfig >> "requiresGen") == 1 ) then
{
...
///tool trolley
if ( getNumber(_recipeConfig >> "requiresToolTrolley") == 1 ) then
{
...
_breakage = floor random _chance;
_toBreak = selectRandom _tools;
if (_breakage == 0) then
{
	if (count _tools >0) then
		{
			_toBreakItemName = getText(configFile >> "CfgMagazines" >> _toBreak >> "displayName");
			["InfoTitleAndText",["Tool Removed", format["Your %1 broke and has been removed from your items!",_toBreakItemName]]] call ExileClient_gui_toaster_addTemplateToast;
		};
	player removeItem _toBreak;
};
true