 ...
 */
 ...
 // Modified to make it an MoneyDrop mission by GolovaRaoul, all credits to Exile Mod Team
 // Credtis to the orignal idea, @Aidem
 // https://epochmod.com/forum/topic/3779-4-types-of-side-missions-events/
 
private["_randomPositions","_marker","_config","_markerTime","_filteredAirportPositions","_dropAltitude","_dropRadius","_boxType","_randomPosition","_position","_marker","_box","_itemClassName","_itemCount","_i","_supplySmoke","_parachute"];
_randomPositions = call ExileClient_util_world_getAllAirportPositions;
_config = configFile >> "CfgSettings" >> "Events" >> "MoneyDrop";
_markerTime = getNumber (_config >> "markerTime");
if (_randomPositions isEqualTo []) exitWith
{
	"The current map has no airports. Skipping money drop box drop event." call ExileServer_util_log;
};
...
{
	"The current map has airports, but they are too close to traders. Skipping money drop box drop event." call ExileServer_util_log;
};
_config = configFile >> "CfgSettings" >> "Events" >> "MoneyDrop";
...
["toastRequest", ["InfoTitleAndText", ["Money Drop", "The Bank has lost it's money while trying to transfer it, Check the Yellow Circle on your Map for the Location!"]]] call ExileServer_system_network_send_broadcast;
/*
 * Exile Marker
...
forEach (getArray (_config >> "BoxTypes4" >> _boxType >> "items"));
