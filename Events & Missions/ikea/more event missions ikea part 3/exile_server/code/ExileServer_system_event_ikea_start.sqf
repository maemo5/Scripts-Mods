 ...
 */
 ...
 // Modified to make it an IKEA mission by GolovaRaoul, all credits to Exile Mod Team
...
_config = configFile >> "CfgSettings" >> "Events" >> "IKEA";
_markerTime = getNumber (_config >> "markerTime");
if (_randomPositions isEqualTo []) exitWith
{
	"The current map has no airports. Skipping ikea box drop event." call ExileServer_util_log;
};
...
["toastRequest", ["InfoTitleAndText", ["IKEA Truck", "Ikea Truck lost its Cargo, Check the Blue Circle on your Map for the Location!"]]] call ExileServer_system_network_send_broadcast;
/*
 * Exile Marker
...
forEach (getArray (_config >> "BoxTypes2" >> _boxType >> "items"));
