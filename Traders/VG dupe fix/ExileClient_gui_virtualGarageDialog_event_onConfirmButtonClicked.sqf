	...
	switch (_index) do
	{
		...
		case 1:
		{
			...
			if (getNumber(missionConfigFile >> "CfgVirtualGarage" >> "clearInventoryOnStore") isEqualTo 1) then 
			{
				...
				{
					_result = ["The vehicle you are about to store has items/poptabs in it's inventory. Items will be dropped on the ground and poptabs deleted upon storing the vehicle!<br />Are you sure you want to store your vehicle?", "Warning", true, true] call BIS_fnc_guiMessage;
					if !(_result) then 
					{
						throw "Cancelled";
					};
