...
else
{
	...
	format["insertPlayerHistory:%1:%2:%3:%4:%5:%6", getPlayerUID _victim, name _victim, _victimPosition select 0, _victimPosition select 1, _victimPosition select 2, worldName] call ExileServer_system_database_query_fireAndForget;
	format["deletePlayer:%1:%2", _victim getVariable ["ExileDatabaseId", -1], worldName] call ExileServer_system_database_query_fireAndForget;
};
true