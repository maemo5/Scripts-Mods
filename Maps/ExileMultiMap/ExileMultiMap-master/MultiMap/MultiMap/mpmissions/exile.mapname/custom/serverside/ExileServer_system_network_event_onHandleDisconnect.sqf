...
if !(_uid in ["", "__SERVER__", "__HEADLESS__"]) then
{
	...
	if !(_unit getVariable ["ExileIsDead", false]) then
	{
		...
		{
			_unit setDamage 999;
			format["insertPlayerHistory:%1:%2:%3:%4:%5:%6", _uid, _name, _pos select 0, _pos select 1, _pos select 2, worldName] call ExileServer_system_database_query_fireAndForget;
			format["deletePlayer:%1:%2", _unit getVariable ["ExileDatabaseId", -1], worldName] call ExileServer_system_database_query_fireAndForget;
